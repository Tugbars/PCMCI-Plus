...
#pragma omp simd
    ...
    /**
     * Compute ranks of array (for Spearman correlation)
     * Handles ties with average rank.
     *
     * @param x     Input array, length n
     * @param n     Number of elements
     * @param out   Output ranks, length n (pre-allocated)
     */
    void pcmci_rank_into(const double *x, int32_t n, double *out);

    /**
     * Winsorize array: clip extreme values to percentile thresholds
     *
     * @param x         Input array, length n
     * @param n         Number of elements
     * @param thresh    Percentile threshold (e.g., 0.01 for 1%/99%)
     * @param out       Output array, length n (pre-allocated)
     */
    void pcmci_winsorize_into(const double *x, int32_t n, double thresh, double *out);

    /**
     * Create a rank-transformed copy of entire dataframe
     * Used for Spearman partial correlation.
     *
     * @param df    Original dataframe
     * @return      New dataframe with ranked data (caller must free)
     */
    pcmci_dataframe_t *pcmci_dataframe_to_ranks(const pcmci_dataframe_t *df);

    /**
     * Create a winsorized copy of entire dataframe
     *
     * @param df        Original dataframe
     * @param thresh    Percentile threshold (e.g., 0.01 for 1%/99%)
     * @return          New dataframe with winsorized data (caller must free)
     */
